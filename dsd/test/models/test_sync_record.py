...
class SyncRecordTest(TestCase):
    ...
    def test_should_get_last_successful_time(self):
        SyncRecordFactory(time_start=datetime(2016, 8, 31, 2, 25, 0, 0, timezone.utc),
                          created=datetime(2016, 8, 31, 2, 30, 0, 0, timezone.utc), status='Success')
        SyncRecordFactory(created=datetime(2016, 8, 31, 3, 0, 0, 0, timezone.utc), status='Failure')

        time = SyncRecord.get_last_successful_sync_start_time()
        self.assertEqual(time, datetime(2016, 8, 31, 2, 25, 0, 0, timezone.utc))

        SyncRecordFactory(time_start=datetime(2016, 8, 31, 3, 19, 0, 0, timezone.utc),
                          created=datetime(2016, 8, 31, 3, 30, 0, 0, timezone.utc), status='Success')
        ...
        time = SyncRecord.get_last_successful_sync_start_time()
        self.assertEqual(time, datetime(2016, 8, 31, 3, 19, 0, 0, timezone.utc))

        SyncRecordFactory(time_start=None,
                          created=datetime(2016, 8, 31, 5, 30, 0, 0, timezone.utc), status='Success')

        SyncRecordFactory(created=datetime(2016, 8, 31, 6, 30, 0, 0, timezone.utc), status='Failure')
